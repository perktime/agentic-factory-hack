...
public sealed class FaultMappingService : IFaultMappingService
{
    ...
    private static readonly IReadOnlyList<string> NoParts = [];
    ...
    {
        return !string.IsNullOrWhiteSpace(faultType) && FaultToSkills.TryGetValue(faultType, out var skills)
            ? skills
            : DefaultSkills;
    }
    ...
    {
        return !string.IsNullOrWhiteSpace(faultType) && FaultToParts.TryGetValue(faultType, out var parts)
            ? parts
            : NoParts;
    }
}